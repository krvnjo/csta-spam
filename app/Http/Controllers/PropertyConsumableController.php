<?php

namespace App\Http\Controllers;

use App\Models\PropertyConsumable;
use Illuminate\Http\Request;

class PropertyConsumableController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        return view('pages.property-asset.consumable.overview-consumable');
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(PropertyConsumable $propertyConsumable)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(PropertyConsumable $propertyConsumable)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, PropertyConsumable $propertyConsumable)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(PropertyConsumable $propertyConsumable)
    {
        //
    }
}
