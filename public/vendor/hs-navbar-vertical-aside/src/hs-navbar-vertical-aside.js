...
export default class HSSideNav {
  ...
  init() {
    ...
    this.collapseList.forEach((collapse) => {
      collapse._element.addEventListener("show.bs.collapse", (e) => {
        ...
        setTimeout(() => {
          if (
            ($mainContainer.classList.contains(this.settings.minClass.slice(1)) || this.isCompact) &&
            parentEl &&
  ...
  toggleSubMenu($invoker) {
    ...
    if ((collapseOthers && onAction) || (collapseOthers && topLevel && mini)) {
      ...
      allExcludeTarget
        ...
        .forEach(($item) => {
          slideUp($item, this.settings.subMenuAnimationSpeed).parentNode.classList.remove(this.settings.subMenuInvokerActiveClass.slice(1));
        });
    }
  ...
  toggleSidebar() {
    ...
    if (
      (!this.showResolutionChecking() && $mainContainer.classList.contains(this.settings.minClass.slice(1))) ||
      (this.showResolutionChecking() && $mainContainer.classList.contains(this.settings.closedClass.slice(1)))
    ) {
